...
def read(soc_serv,msg_params_list, addr):
    ...
    try:
        ...
        if err == FileNotFoundError :
            create_msg_and_send_it(ERROR, [ERROR, "01", "File not found", "0"])
        elif err == PermissionError :
            create_msg_and_send_it(ERROR, [ERROR, "02", "Access to file denied", "0"])
        else:
            create_msg_and_send_it(ERROR, [ERROR, "00", "Unknown error while opening the file", "0"])
        return
    got_to_eof = 0
    while got_to_eof == 0 :
        block_number += 1
...
def write(soc_serv, addr, path, adder):
    new_data_params = [ACK, "0", ""]
    try:
        f = open(msg_params_list[1], 'a')
    except OSError as err:
        print(err.stderror)
        if err == FileExistsError:
            create_msg_and_send_it(ERROR, [ERROR, "06", "File already exists", "0"])
        elif err == PermissionError:
            create_msg_and_send_it(ERROR, [ERROR, "02", "Access to file denied", "0"])
        else:
            create_msg_and_send_it(ERROR, [ERROR, "00", "unknown error while opening the file", "0"])
...
# this function return an array of all the messaged params as declared in the protocol
def parser(msg):
    params_list = ["" for i in range(5)]
    # neet to handle the case of empty packet
    return params_list
